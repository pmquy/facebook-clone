...
export default function Home() {
  ...
  clickOutSide([ref1, parentRef1], () => setIsComposing(false));
  ...
  useEffect(() => {  
    if(isComposing) {
      ...
      if(ref1.current && inputRef.current)
  ...
  const createPost = async (e) => {
    ...
    mutation.mutate({text : inputRef.current.value, userId : user._id, img : imgId});    
          ...
          <div className={`${isDark ? 'brightness-50 pointer-events-none' : ''} flex flex-row justify-center`}>              
            <div className='lg:basis-6/12 flex flex-col basis-11/12'>          
              <div className={`p-4 bg-blue-600 rounded-lg my-5 flex z-10 sticky ${isHeaderHidden ? 'top-0 ' : 'top-16'} items-center transition-all flex-row gap-5`}>
                <UserImage user={user}/>
                <input ref={ref1} onClick={() => setIsComposing(true)} readOnly className=' indent-4 h-full text-white outline-none placeholder:text-white w-full rounded-3xl bg-stone-400' placeholder={`${user.username} ơi, bạn đang nghĩ gì thế?`}></input>   
              </div>

              {postsQuery.data.map(element => <div key={element._id} className='my-5'>
                <Post post={element}/>
              </div>)}
            </div>
          </div>
          
          <div ref={parentRef1} className={`${isComposing ? '' : 'hidden'} z-20 fixed w-4/6 max-h-[80vh] overflow-y-scroll  rounded-lg bg-zinc-600 p-10 top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2`}>
            ...
            <form className='w-full my-2 flex flex-col' id='myForm' onSubmit={createPost}>
              <textarea name='fdf' ref={inputRef} rows={5} className='my-2 indent-4 rounded-lg  text-white outline-none placeholder:text-white w-full bg-stone-400'></textarea>              
